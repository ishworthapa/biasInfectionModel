;; biasInfection model
;; Dario Ghersi and Ishwor Thapa
;; Last updated: 030323

;; agent sets
breed [ humans human ]
breed [ mosquitoes mosquito ]

;; global variables
globals [ infected-at-start ]

;; attributes of human agents
humans-own
[
  infected?
  ;; after an individual is recovered, it can get infected again
  recovered?
  ;; for infected individuals, add sick-time with the tick
  sick-time

  recovery-time
  time-sick
  time-recovered

  time-elapsed-before-bitten-by-infected
  time-became-susceptible
]

;; attributes of mosquito agents
mosquitoes-own
[
  exposed?
  ;; after exposed, incubation time starts
  incubation-time
  time-incubation
  ;; after incubation, becomes carrier
  carrier?
  carrier-time
  time-carrier
  closest-human
  time-elapsed-before-biting-infected
  birth-time
]

to-report infected-humans
  report humans with [infected? = true]
end

to-report recovered-humans
  report humans with [recovered? = true]
end

to-report exposed-mosquitoes
  report mosquitoes with [exposed? = true]
end

to-report carrier-mosquitoes
  report mosquitoes with [carrier? = true]
end

to-report max-time-before-biting-infected
  report [time-elapsed-before-biting-infected] of max-one-of mosquitoes [ time-elapsed-before-biting-infected]
end

to-report median-time-before-biting-infected
  ifelse any? mosquitoes with [time-elapsed-before-biting-infected >= 0] [
    report median [time-elapsed-before-biting-infected] of mosquitoes with [time-elapsed-before-biting-infected >= 0]
  ] [report -1]
end

to-report mean-time-before-biting-infected
  ifelse any? mosquitoes with [time-elapsed-before-biting-infected >= 0] [
    report mean [time-elapsed-before-biting-infected] of mosquitoes with [time-elapsed-before-biting-infected >= 0]
  ] [report -1]
end

to-report min-time-before-biting-infected
  ifelse any? mosquitoes with [time-elapsed-before-biting-infected >= 0] [
    report min [time-elapsed-before-biting-infected] of mosquitoes with [time-elapsed-before-biting-infected >= 0]
  ] [report -1]
end

to-report median-time-before-bitten-by-infected
  ifelse any? humans with [time-elapsed-before-bitten-by-infected >= 0] [
    report median [time-elapsed-before-bitten-by-infected] of humans with [time-elapsed-before-bitten-by-infected >= 0]
  ] [report -1]
end

to-report mean-time-before-bitten-by-infected
  ifelse any? humans with [time-elapsed-before-bitten-by-infected >= 0] [
    report mean [time-elapsed-before-bitten-by-infected] of humans with [time-elapsed-before-bitten-by-infected >= 0]
  ] [report -1]
end

;; setup procedure
to setup
  clear-all
  ;; fix the random-seed for reproducibility
  ;random-seed behaviorspace-run-number
  ask patches [ set pcolor gray + 4 ]
  set-default-shape humans "person"
  set-default-shape mosquitoes "bug"

  ;; create humans
  create-humans num-of-individuals [
    set color green
    set infected? false
    set recovered? false
    setxy random-xcor random-ycor
    ;set transmit-count 0
    set time-became-susceptible 0
    set time-elapsed-before-bitten-by-infected -1
  ]

  ;; create mosquitos
  create-mosquitoes num-of-mosquitoes [
    setxy random-xcor random-ycor
    set color blue
    set carrier? false
    set exposed? false
    set time-carrier 0
    set time-incubation 0
    set time-elapsed-before-biting-infected -1
    set birth-time 0
  ]

  set infected-at-start %-of-infected-at-start * num-of-individuals / 100

  ask n-of infected-at-start humans [
    set color red
    set infected? true

    ;; set up the sick time from a normal distribution
    set sick-time round(random-normal average-sick-time (average-sick-time / 4))
    if (sick-time < 0) [set sick-time 0]
    set time-sick 0
    output-print sick-time
  ]
  reset-ticks
end

;; go procedure
to go

  ;; stopping criterion
  if all? humans [ recovered? = true or infected? = false ] and
     all? mosquitoes [ carrier? = false and exposed? = false] [
    stop
  ]

  ;; update sick time
  ask infected-humans [
    set time-sick time-sick + 1
  ]

  ;; update recovered time
  ask recovered-humans [
    set time-recovered time-recovered + 1
  ]

  ask mosquitoes with [ exposed? = true ] [
    set time-incubation time-incubation + 1
  ]

  ask mosquitoes with [ carrier? = true ] [
    set time-carrier time-carrier + 1
  ]

  ;; check if a patient should recover (still has immunity)
  ask infected-humans with [ time-sick = sick-time ] [
    set color brown
    set infected? false
    set recovered? true
    set recovery-time round(random-normal average-recovery-time (average-recovery-time / 4))
    if (recovery-time < 0) [set recovery-time 0]
    set time-recovered 0
    set time-sick 0
  ]

  ;; check if a patient should become susceptible again (immunity lost)
  ask recovered-humans with [ time-recovered = recovery-time ] [
    set color green
    set recovered? false
    set time-recovered 0
    set time-became-susceptible ticks
    set time-elapsed-before-bitten-by-infected -1

    ;set transmit-count 0
  ]

  ;; check if a mosquito should become carrier
  ask exposed-mosquitoes with [ time-incubation = incubation-time ] [
    set color pink
    set carrier? true
    set exposed? false
    set carrier-time round(random-normal average-carrier-time (average-carrier-time / 4))
    if (carrier-time < 0) [set carrier-time 0]
    set time-carrier 0
    set time-incubation 0
  ]


  ;; check if a mosquito should revert to non-carrier
  ;; consider this is the end of life for the mosquitoe and new one replaces this
  ask carrier-mosquitoes with [ time-carrier = carrier-time ] [
    set color blue
    set carrier? false
    set time-carrier 0
    set birth-time ticks
    set time-elapsed-before-biting-infected -1
  ]


  ;; move humans randomly
  ask humans [
    set heading (random 360)
    fd 1
  ]

  ;; move mosquitos
  ask mosquitoes [

    mosquito-infect-human
    mosquito-bite-infected

    ifelse random-float 1 < human-attractiveness [
      ifelse random-float 1 < bias and any? infected-humans [
        set closest-human min-one-of infected-humans[ distance myself ]
        set heading (towards closest-human)
        fd 1
      ]
      [
        set closest-human min-one-of humans [ distance myself ]
        set heading (towards closest-human)
        fd 1
      ]
    ]
    [
      set heading (random 360)
      fd 1
    ]
  ]

  tick
end

;; mosquitoes infect a human
to mosquito-infect-human
  if count humans-here with [infected? = false and recovered? = false] > 0 and
  count mosquitoes-here with [carrier? = true] > 0 [
    ;; find one who is not infected and not in recovery (meaning having immunity)
    ask one-of humans-here with [ infected? = false and recovered? = false ] [
      set infected? true
      set time-elapsed-before-bitten-by-infected ticks - time-became-susceptible
      set color red
      set sick-time round(random-normal average-sick-time (average-sick-time / 4))
      if (sick-time < 0) [set sick-time 0]
      set time-sick 0
    ]
  ]
end

;; mosquitoes bite infected individuals
to mosquito-bite-infected
  if count humans-here with [infected? = true] > 0 and
  count mosquitoes-here with [exposed? = false and carrier? = false] > 0 [

    ask one-of mosquitoes-here with [exposed? = false and carrier? = false] [
      set exposed? true
      set time-elapsed-before-biting-infected ticks - birth-time


      set incubation-time round(random-normal average-incubation-time (average-incubation-time / 4))
      if (incubation-time < 0 ) [set incubation-time 0]
      set color black
      set time-incubation 0
    ]
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
628
13
1184
570
-1
-1
16.61
1
10
1
1
1
0
1
1
1
-16
16
-16
16
1
1
1
ticks
30.0

BUTTON
25
10
95
43
NIL
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

BUTTON
140
10
210
43
go-once
go
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SLIDER
323
130
612
163
bias
bias
0
1
0.2
0.05
1
NIL
HORIZONTAL

PLOT
25
227
310
396
Total Humans Infected
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count humans with [infected? = true]"

SLIDER
323
11
610
44
num-of-individuals
num-of-individuals
1
200
200.0
1
1
NIL
HORIZONTAL

SLIDER
323
51
610
84
num-of-mosquitoes
num-of-mosquitoes
1
200
200.0
1
1
NIL
HORIZONTAL

SLIDER
323
91
611
124
%-of-infected-at-start
%-of-infected-at-start
0
100
10.0
1
1
NIL
HORIZONTAL

BUTTON
242
10
308
43
go
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SLIDER
25
50
310
83
average-sick-time
average-sick-time
1
100
5.0
1
1
NIL
HORIZONTAL

SLIDER
25
89
310
122
average-recovery-time
average-recovery-time
1
100
30.0
1
1
NIL
HORIZONTAL

PLOT
322
228
615
397
Total Carrier/Exposed Mosquitoes
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count mosquitoes with [carrier? = true]"
"pen-1" 1.0 0 -955883 true "" "plot count mosquitoes with [exposed? = true]"

SLIDER
25
127
310
160
average-carrier-time
average-carrier-time
1
100
21.0
1
1
NIL
HORIZONTAL

SLIDER
323
170
613
203
human-attractiveness
human-attractiveness
0
1
0.2
0.05
1
NIL
HORIZONTAL

MONITOR
323
524
540
569
# of Infected Humans
count infected-humans
0
1
11

MONITOR
322
412
537
457
# of Carrier Mosquitoes
count mosquitoes with [carrier? = true]
17
1
11

SLIDER
25
167
310
200
average-incubation-time
average-incubation-time
1
100
7.0
1
1
NIL
HORIZONTAL

MONITOR
323
470
538
515
# of Mosquitoes in incubation
count exposed-mosquitoes
17
1
11

MONITOR
26
469
241
514
# of  Humans in full recovery process
count recovered-humans
17
1
11

MONITOR
25
413
240
458
Total Viral Load
count infected-humans + count carrier-mosquitoes + count exposed-mosquitoes
17
1
11

MONITOR
25
525
240
570
max-time-before-biting-infected
[time-elapsed-before-biting-infected] of max-one-of mosquitoes [time-elapsed-before-biting-infected ]
17
1
11

@#$#@#$#@
## WHAT IS IT?

This is a simple model with Mosquito and Human as agents. It includes two aspects of viral propagation in vector borne diseases, viz. the mosquito's attractiveness to human in general and further preferential attractiveness to infected hosts. 

## HOW IT WORKS
In every tick, the mosquitoes and humans can move one step. Human agents can move in any direction randomly, while the direction of mosquito depend on attractiveness and bias ( preferential attractiveness to infected hosts) parameter values. When an uninfected mosquito share a patch with an infected host, the mosquito is inoculated with the virus and its state changes to incubation stage. After incubation time is finished, the mosquito will change to a carrier state and can infect an uninfected human. The mosquito remains to be in carrier state for its lifetime and after its lifetime (given by average carrier time), the mosquito changes to initial uninfected state, representing replacement of previous mosquito by a newly born one. When a human agent is infected by biting of carrier mosquito, its state converts to infected stage and is sick for a period of time. After the sick time is over, the individual can no longer transmit the virus to another mosquito and can not get re-infected for a period of time. 

## HOW TO USE IT

Set the num-of-mosquitoes, number-of-individuals and %-of-infected-at-start. The 'human-attractiveness' and 'bias' parameters can be set between 0 to 1. 

Click on the SETUP button to set up the world with Mosquito and Human. Click on GO to start the model. The human and mosquitoes can wrap around the world as they move across the boundaries. 

## THINGS TO TRY
Set num-of-individuals = 100, num-of-mosquitoes = 100 and %-of-infected-at-start = 10

Set different values for attractiveness and bias and RUN the model. Try out with human-attractiveness = 0.2 or 0.6 or 1.0​ and bias = 0.0 or 0.2 or 0.6 or 1.0​.


## CREDITS AND REFERENCES

https://github.com/ishworthapa/biasInfectionModel
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

sheep
false
15
Circle -1 true true 203 65 88
Circle -1 true true 70 65 162
Circle -1 true true 150 105 120
Polygon -7500403 true false 218 120 240 165 255 165 278 120
Circle -7500403 true false 214 72 67
Rectangle -1 true true 164 223 179 298
Polygon -1 true true 45 285 30 285 30 240 15 195 45 210
Circle -1 true true 3 83 150
Rectangle -1 true true 65 221 80 296
Polygon -1 true true 195 285 210 285 210 240 240 210 195 210
Polygon -7500403 true false 276 85 285 105 302 99 294 83
Polygon -7500403 true false 219 85 210 105 193 99 201 83

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

wolf
false
0
Polygon -16777216 true false 253 133 245 131 245 133
Polygon -7500403 true true 2 194 13 197 30 191 38 193 38 205 20 226 20 257 27 265 38 266 40 260 31 253 31 230 60 206 68 198 75 209 66 228 65 243 82 261 84 268 100 267 103 261 77 239 79 231 100 207 98 196 119 201 143 202 160 195 166 210 172 213 173 238 167 251 160 248 154 265 169 264 178 247 186 240 198 260 200 271 217 271 219 262 207 258 195 230 192 198 210 184 227 164 242 144 259 145 284 151 277 141 293 140 299 134 297 127 273 119 270 105
Polygon -7500403 true true -1 195 14 180 36 166 40 153 53 140 82 131 134 133 159 126 188 115 227 108 236 102 238 98 268 86 269 92 281 87 269 103 269 113

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.3.0
@#$#@#$#@
@#$#@#$#@
@#$#@#$#@
<experiments>
  <experiment name="experiment3_100H200M_rs_pcInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="200"/>
    </enumeratedValueSet>
    <steppedValueSet variable="%-of-infected-at-start" first="2" step="4" last="30"/>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_Mosquito_2_4_6_8_10_pc_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="500"/>
    </enumeratedValueSet>
    <steppedValueSet variable="num-of-mosquitoes" first="10" step="10" last="50"/>
    <enumeratedValueSet variable="%-of-infected-at-start">
      <value value="2"/>
    </enumeratedValueSet>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_100H50M_rs_pcInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="50"/>
    </enumeratedValueSet>
    <steppedValueSet variable="%-of-infected-at-start" first="2" step="4" last="30"/>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_100H100M_rs_pcInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="100"/>
    </enumeratedValueSet>
    <steppedValueSet variable="%-of-infected-at-start" first="2" step="4" last="30"/>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_200H200M_rs_pcInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="200"/>
    </enumeratedValueSet>
    <steppedValueSet variable="%-of-infected-at-start" first="2" step="4" last="30"/>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_200H400M_rs_pcInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="200"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="400"/>
    </enumeratedValueSet>
    <steppedValueSet variable="%-of-infected-at-start" first="2" step="4" last="30"/>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_100H100M_rs_10pcInfection_100rep" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="%-of-infected-at-start">
      <value value="10"/>
    </enumeratedValueSet>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_100H100M_rs_pcInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="100"/>
    </enumeratedValueSet>
    <steppedValueSet variable="%-of-infected-at-start" first="2" step="4" last="30"/>
    <steppedValueSet variable="bias" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0" step="0.1" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment3_100H100M_rs_10pc0.9BHInfection_100rep" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <metric>count infected-humans</metric>
    <metric>count carrier-mosquitoes</metric>
    <metric>count exposed-mosquitoes</metric>
    <metric>max-time-before-biting-infected</metric>
    <metric>min-time-before-biting-infected</metric>
    <metric>mean-time-before-biting-infected</metric>
    <metric>median-time-before-biting-infected</metric>
    <metric>median-time-before-bitten-by-infected</metric>
    <metric>mean-time-before-bitten-by-infected</metric>
    <enumeratedValueSet variable="num-of-individuals">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-of-mosquitoes">
      <value value="100"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="%-of-infected-at-start">
      <value value="10"/>
    </enumeratedValueSet>
    <steppedValueSet variable="bias" first="0.9" step="0.01" last="1"/>
    <enumeratedValueSet variable="average-sick-time">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-recovery-time">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="average-carrier-time">
      <value value="21"/>
    </enumeratedValueSet>
    <steppedValueSet variable="human-attractiveness" first="0.9" step="0.01" last="1"/>
    <enumeratedValueSet variable="average-incubation-time">
      <value value="7"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
0
@#$#@#$#@
